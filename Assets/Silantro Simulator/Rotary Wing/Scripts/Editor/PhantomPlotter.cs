...
public class PhantomPlotter : EditorWindow
{


	// ----------------------------------------------------------------------------------------------------------------------------------------------------------
	[MenuItem("Oyedoyin/Rotary Wing/Airfoil System/Open", false, 300)]
	public static void ShowWindow()
	{
		GetWindow<PhantomPlotter>("Foil System");
	}

	// ----------------------------------------------------------------------------------------------------------------------------------------------------------
	[MenuItem("Oyedoyin/Rotary Wing/Airfoil System/Tutorial", false, 400)]
	...
	Color PhantomColor;[HideInInspector] public int toolbarTab; public int dataToolbarTab;

	// ----------------------------------------------------------------------------------------------------------------------------------------------------------
	public string identifier = "NACA 00000";
	public string airfoilShapePath;
	public string airfoilPerformancePath;
	public string airfoilPrefabLocation = "Assets/Silantro Simulator/Rotary Wing/Airfoils/";
	...
	void OnGUI()
	{
		PhantomColor = new Color(1, 0.4f, 0);
			...
			case "Create":
				...
				if (airfoilPerformancePath != null && airfoilPerformancePath.Length > 0 && airfoilShapePath != null && airfoilShapePath.Length > 0)
				{
					...
					if (GUILayout.Button("Create Wingfoil"))
					{
						...
						PhantomCreateFoil plotter = manager.AddComponent<PhantomCreateFoil>();
						//SET DATA
						...
						if (fileType == FileType.AFL) { plotter.dataFormat = PhantomCreateFoil.DataFormat.Afl; }
						if (fileType == FileType.TXT) { plotter.dataFormat = PhantomCreateFoil.DataFormat.Txt; }
			...
			case "Edit":
				...
				if (airfoil != null)
				{
					...
					if (GUILayout.Button("Edit Wingfoil"))
					{
						...
						if (fileType == FileType.AFL)
						{
							plotter.dataFormat = PhantomCreateFoil.DataFormat.Afl;
						}
						if (fileType == FileType.TXT)
						{
							plotter.dataFormat = PhantomCreateFoil.DataFormat.Txt;
						}
...
public class PhantomCreateFoil : MonoBehaviour
{
	public string identifier = "NACA 00000";
	public string airfoilShapePath;
	public string airfoilPerformancePath;
