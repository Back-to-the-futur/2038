...
public class PhantomRocketMotor : MonoBehaviour
{
	//---------------------SELECTIBLES
	...
	//----------------------SOLID FUEL
	public enum SolidFuelType { DB, PVC_AP_AL, PBAA_AP_AL, AN_Polymer, HTPB_AP_AL, PB_AN_AP_AL, PU_AP_AI }
	...
	public float a2, b2, c2, w;
	public float altitude, kiloMeter, a, b;
	//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	void MotorAnalysis()
	{
		// ------------------ Ambient
		altitude = transform.position.y;
		kiloMeter = MathBase.ConvertDistance(altitude, "FT");
		a = 0.0000004f * kiloMeter * kiloMeter;
		b = (0.0351f * kiloMeter);
		ambientPressure = (a - b + 1009.6f) / 10f;
		if(ambientPressure > 150) { ambientPressure = 150f; }
		...
		//============================================================NOZZLE
		a2 = (gamma - 1) / gamma;
		b2 = Mathf.Pow(((exitPressure * 1000) / (chamberPressure * 1000)), a2);
		c2 = 2 * cp * combustionTemperature * (1 - b2);
		exhaustVelocity = Mathf.Sqrt(Mathf.Abs(c2));
		...
		momentumThrust = (me * exhaustVelocity); 
		pressureThrust = (((exitPressure * 1000) - (ambientPressure * 1000)) * Ae);
		meanThrust = momentumThrust + pressureThrust;
		Thrust = meanThrust * thrustFactor;
		float tf = w * 10f;
		if(Thrust < 0) { Thrust = 0f; }
		//if(Thrust > tf) { Thrust = tf; }
