...
[Serializable]
public class PhantomFuelSystem
{
    ...
    public List<PhantomFuelTank> internalFuelTanks;
    public List<PhantomFuelTank> auxilaryTanks;


    public List<PhantomFuelTank> RightTanks;
    public List<PhantomFuelTank> LeftTanks;
    public List<PhantomFuelTank> CentralTanks;
    ...
    public void InitializeDistributor()
    {
        ...
        auxilaryTanks = new List<PhantomFuelTank>();
        ...
        // ------------------------ Filter
        foreach (PhantomFuelTank tank in fuelTanks)
        {
            controller.TotalFuelCapacity += tank.actualAmount;
            controller.fuelLevel += tank.CurrentAmount;
            if (tank.tankType == PhantomFuelTank.TankType.Main) { internalFuelTanks.Add(tank); }
            if (tank.tankType == PhantomFuelTank.TankType.Auxillary) { auxilaryTanks.Add(tank); }
            if (tank.fuelType.ToString() != fuelType) { Debug.LogError("Fuel Type Selection not uniform"); return; }
        }
        foreach (PhantomFuelTank tank in internalFuelTanks)
        {
            if (tank.tankPosition == PhantomFuelTank.TankPosition.Center) { CentralTanks.Add(tank); }
            if (tank.tankPosition == PhantomFuelTank.TankPosition.Left) { LeftTanks.Add(tank); }
            if (tank.tankPosition == PhantomFuelTank.TankPosition.Right) { RightTanks.Add(tank); }
        }
    }
    ...
    public void UpdateFuel()
    {
        ...
        if (fuelTanks.Length > 0) { foreach (PhantomFuelTank tank in fuelTanks) { controller.fuelLevel += tank.CurrentAmount; } }
        foreach (PhantomFuelTank tank in fuelTanks) { if (tank.CurrentAmount < 0) { tank.CurrentAmount = 0f; } }
    ...
    void DepleteTanks()
    {
        ...
        controller.totalConsumptionRate = 0f;
        if(controller.engineType == PhantomController.EngineType.Turboshaft)
        {
            if(controller.shafts != null && controller.shafts[0] != null) { controller.isRunning = controller.shafts[0].core.active; }
            foreach(PhantomTurboShaft shaft in controller.shafts) { controller.totalConsumptionRate += shaft.mf; }
        }
        if (controller.engineType == PhantomController.EngineType.Piston)
        {
            if (controller.pistons != null && controller.pistons[0] != null) { controller.isRunning = controller.pistons[0].core.active; }
            foreach (PhantomPistonEngine piston in controller.pistons) { controller.totalConsumptionRate += piston.Mf; }
        }
        ...
        foreach (PhantomFuelTank tank in LeftTanks) { LeftFuelAmount += tank.CurrentAmount; }
        foreach (PhantomFuelTank tank in RightTanks) { RightFuelAmount += tank.CurrentAmount; }
        foreach (PhantomFuelTank tank in CentralTanks) { CenterFuelAmount += tank.CurrentAmount; }
        foreach (PhantomFuelTank tank in auxilaryTanks) { ExternalFuelAmount += tank.CurrentAmount; }
        ...
        {
            if (auxilaryTanks != null && auxilaryTanks.Count > 0)
            {
                if (ExternalFuelAmount <= 0) { fuelSelector = FuelSelector.Automatic; }
                float individualRate = engineConsumption / auxilaryTanks.Count;
                foreach (PhantomFuelTank tank in auxilaryTanks) { tank.CurrentAmount -= individualRate * timeFactor; }
            }
        ...
        if (fuelSelector == FuelSelector.Left)
        {
            if (LeftTanks != null && LeftTanks.Count > 0)
            {
                ...
                foreach (PhantomFuelTank tank in LeftTanks) { tank.CurrentAmount -= individualRate * timeFactor; }
        ...
        if (fuelSelector == FuelSelector.Right)
        {
            if (RightTanks != null && RightTanks.Count > 0)
            {
                ...
                foreach (PhantomFuelTank tank in RightTanks) { tank.CurrentAmount -= individualRate * timeFactor; }
        ...
        if (fuelSelector == FuelSelector.Automatic)
        {
            ...
            if (CentralTanks != null && CentralTanks.Count > 0 && CenterFuelAmount > 0)
            {
                ...
                foreach (PhantomFuelTank tank in CentralTanks)
                {
            ...
            else
            {
                ...
                if (auxilaryTanks != null && auxilaryTanks.Count > 0 && ExternalFuelAmount > 0)
                {
                    //DEPLETE
                    float individualRate = engineConsumption / auxilaryTanks.Count;
                    foreach (PhantomFuelTank tank in auxilaryTanks)
                    {
                        tank.CurrentAmount -= individualRate * timeFactor;
                    }
                }
                ...
                else
                {
                    ...
                    //LEFT
                    foreach (PhantomFuelTank tank in LeftTanks)
                    {
                        tank.CurrentAmount -= individualRate * timeFactor;
                    }
                    //RIGHT
                    foreach (PhantomFuelTank tank in RightTanks)
                    {
                        tank.CurrentAmount -= individualRate * timeFactor;
                    }
                }
