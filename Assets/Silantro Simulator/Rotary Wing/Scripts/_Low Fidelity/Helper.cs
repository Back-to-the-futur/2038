...
namespace Oyedoyin.Rotary.LowFidelity
{
    ...
    public class Helper
    {
        ...
        // ----------------------------------------------------------------------------------------------------------------------------------------------------------
        public void ToggleLightState() { if (controller != null && controller.isControllable) { foreach (PhantomBulb light in controller.bulbs) { if (light.state == PhantomBulb.CurrentState.On) { light.SwitchOff(); } else { light.SwitchOn(); } } } }
        public void TurnOffLights() { if (controller != null && controller.isControllable) { foreach (PhantomBulb light in controller.bulbs) { if (light.state == PhantomBulb.CurrentState.On) { light.SwitchOff(); } } } }
...
namespace Oyedoyin.Rotary.LowFidelity
{
    ...
    public class MouseControl
    {
        ...
        public PhantomPID pitchSolver;
        public PhantomPID rollSolver;
        public PhantomPID yawSolver;
        ...
        public void m_update()
        {
            ...
            if (m_controller != null)
            {
                ...
                m_camera_pivot.rotation = MathR.Damp(m_camera_pivot.rotation, Quaternion.LookRotation(m_target_object.forward, m_velocity), m_damp_speed, Time.deltaTime);
                ...
                {
                    pitchSolver.Kp = m_pitch_gain.x;
                    pitchSolver.Ki = m_pitch_gain.y;
                    pitchSolver.Kd = m_pitch_gain.z;

                    yawSolver.Kp = m_yaw_gain.x;
                    yawSolver.Ki = m_yaw_gain.y;
                    yawSolver.Kd = m_yaw_gain.z;


                    rollSolver.Kp = m_roll_gain.x;
                    rollSolver.Ki = m_yaw_gain.y;
                    rollSolver.Kd = m_yaw_gain.z;

                    m_pitch = pitchSolver.CalculateOutput(m_pitch_error, Time.deltaTime);
                    m_yaw = yawSolver.CalculateOutput(m_yaw_error, Time.deltaTime);
                }
